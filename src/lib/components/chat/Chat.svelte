<script lang="ts">
	...
	const sendPromptGraphAI = async (model, userPrompt, responseMessageId, _chatId) => {
		...
		try {
			...
			const s3FileAgent = s3FileDummyAgentGenerator(__dirname + "../../tne-agent-v2/src/utils/agentFilter");
			const graphai = new GraphAI(
					iterativeAnalysis,
					{
						...llm_agents,
						...vanilla_agents,
						s3FileAgents: agentInfoWrapper(s3FileAgent),
